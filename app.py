    ...
    if not style_key:
        ...
        try:
            ...
            if style_key == "심플/미니멀 심볼 (아이콘)":
                ...
                formula = f"minimal line logo of a [SUBJECT], vector, {base_keywords} {base_negative}"
            ...
            [절대 지켜야 할 규칙]:
            1. 결과물에 '/imagine prompt:' 라는 문구를 절대 포함하지 마라. 오직 프롬프트 내용만 출력하라.
            2. [ ]로 표시된 부분을 사용자의 의뢰 내용에 맞게 영어로 번역해서 채워 넣어라.
            ...
            with st.spinner("AI가 최적의 프롬프트를 설계 중입니다..."):
                ...
                # [후처리] 혹시 모를 불필요한 문구 강제 제거
                final_prompt = final_prompt.replace("/imagine prompt:", "").replace("`", "").strip()
